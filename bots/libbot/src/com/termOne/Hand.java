...
public class Hand {
	...
	public String toDatabaseString() {
		...
		counter++;
		if(new Hand(ans).getMaxValueCard(seen) != null && new Hand(ans).getMaxValueCard(seen).getSuit().charAt(0) != seen.get(seen.size()-1).getSuit().charAt(0)) {
	...
	private Card getMaxValueCard(ArrayList<Card> exclusion) {
		...
		ArrayList<Card> _cardList = new ArrayList<Card>(this.cardList);
		_cardList.removeAll(exclusion);
		...
		for(Card c : _cardList) {
			if(c.getValue() > card.getValue()) {
				...
				if(this.get_num_suit(this.toString(), c.getSuitChar()) > 
				this.get_num_suit(this.toString(), card.getSuitChar())) {
					card = c;
				}
				else if(this.get_num_suit(this.toString(), c.getSuitChar()) == 
						this.get_num_suit(this.toString(), card.getSuitChar())) {
					if(this.get_num_suit(this.toString(), c.getSuitChar()) == 2) {
						ArrayList<Card> _exclusion = new ArrayList<Card>(exclusion);
						_exclusion.add(c);
						_exclusion.add(card);
						try {
						card = getMaxValueCard(_exclusion).getSuitChar() == c.getSuitChar() ? c : card;
						} catch(NullPointerException e) { } //occurs when _exclusion is all 4 and there are two pairs in hand
						}
					else card = c;
				}
			}
